...
void *cyttsp_core_init(struct cyttsp_bus_ops *bus_ops, struct device *dev,
	...
	if ( (retval = gpio_direction_output(
			...
			pr_err("%s: Failed to setup %d(ENABLE) for output.\n",
			__func__, ts->platform_data->gpio_enable);
		goto error_gpio;
	}
	gpio_set_value(ts->platform_data->gpio_enable, 1);

	gpio_request(ts->platform_data->gpio_reset,"touch_rst");
	if ( (retval = gpio_direction_output(
			ts->platform_data->gpio_reset, 1))) {
			pr_err("%s: Failed to setup %d(RST) for output.\n",
			__func__, ts->platform_data->gpio_reset);
		goto error_gpio;
	}
	gpio_set_value(ts->platform_data->gpio_reset, 1);
	...
	if ( (retval = gpio_direction_input(
				ts->platform_data->gpio_interrupt))) {
