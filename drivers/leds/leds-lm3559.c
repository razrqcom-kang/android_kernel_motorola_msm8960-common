						...
						const char *buf, size_t count)
{
	...
	if (torch_val) {
		...
		err = lm3559_read_reg(torch_data, LM3559_ENABLE_REG, &val);
		if (err) {
			...
			const struct i2c_device_id *id)
{
	...
	if (torch_data->pdata->flags & LM3559_MSG_IND) {
		if (lm3559_debug)
			...
			__func__);
		torch_data->msg_ind_red_class_dev.name = "msg_red";
		...
		torch_data->msg_ind_green_class_dev.name = "msg_green";
		...
		torch_data->msg_ind_blue_class_dev.name = "msg_blue";
