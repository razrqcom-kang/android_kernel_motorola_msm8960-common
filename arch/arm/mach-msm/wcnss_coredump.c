			...
			loff_t *pos)
{
	...
	size_t copy_size = 0;
	int ret = 0;
	const char *coredump_mem = "RivaPanic";
	...
	memcpy(device_mem, coredump_mem, copy_size - 1);
...
const struct file_operations riva_coredump_file_ops = {
	.open = coredump_open,
...
void *create_riva_coredump_device(const char *dev_name)
{
	...
	rd_dev->device.fops = &riva_coredump_file_ops;
