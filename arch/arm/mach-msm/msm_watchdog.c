...
static void wdog_disable_work(struct work_struct *work)
{
	...
	atomic_notifier_chain_unregister(&touch_watchdog_notifier_head, &touch_nmi_blk);
...
// FIXME-HASH: need to re-establish the extended ramconsole buffer
#if 0
#define MSMWDTD(fmt, args...) do {		\
	ram_console_ext_oldbuf_print(fmt, ##args);	\
} while (0)
#endif
#define MSMWDTD(fmt, args...) do {} while (0)
